...
import { RecruiterRegistrationForm } from "@/types/auth";
import {
  EMAIL_PATTERN,
  PHONE_PATTERN,
  PASSWORD_PATTERN,
...
export default function RecruiterRegistrationPage() {
  ...
  const [form, setForm] = useState<RecruiterRegistrationForm>({
    companyName: "",
    firstName: "",
    lastName: "",
    designation: "",
  ...
  const [buttonText, setButtonText] = useState("Create Recruiter Account →");
  ...
  const handleChange = (field: keyof RecruiterRegistrationForm, value: string | boolean) => {
      ...
      <section className="w-full max-w-[620px] p-[35px_40px] max-[600px]:p-[28px_20px] rounded-[22px] max-[600px]:rounded-[18px] border border-[#1b304b] bg-gradient-to-br from-[rgba(15,28,46,0.97)] to-[rgba(8,18,32,0.97)] shadow-[0_25px_80px_rgba(0,0,0,0.35)] backdrop-blur-[20px]">
        ...
        <div className="text-center mb-[27px]">
          ...
          <p className="text-[#4b94ff] text-[11px] font-bold tracking-[1.5px] mb-[8px] uppercase">
            {step === 1 ? "RECRUITER REGISTRATION" : "VERIFY EMAIL"}
          </p>
          <h1 className="text-[28px] max-[600px]:text-[24px] font-extrabold mb-[9px] text-[#f4f7fb]">
            {step === 1 ? "Register your company" : "Check your email"}
          </h1>
          <p className="text-[#8fa2bb] text-[13px] leading-[1.6]">
            {step === 1 ? (
              "Create a recruiter account to connect with talented students and manage placement opportunities."
            ) : (
              <>
        ...
        {isSuccess && (
          <FormSuccessAlert
            ...
            message="Your recruiter account has been verified. Redirecting to login..."
          />
        )}
          ...
          <form onSubmit={handleRegisterSubmit} noValidate>
            ...
            <div className="mb-[17px]">
              ...
              <small className="block mt-[6px] text-[#61748c] text-[10px]">
                Use your company/official email address.
              </small>
            </div>
